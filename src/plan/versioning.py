    ...
    def _init_schema(self):
        ...
        with self.lock:
            with self.conn:
                ...
                """)
                self.conn.execute("CREATE INDEX IF NOT EXISTS idx_version_lamport ON plan_versions(lamport)")
                self.conn.execute("CREATE INDEX IF NOT EXISTS idx_version_merkle ON plan_versions(merkle_root)")
                ...
                """)
                self.conn.execute("CREATE INDEX IF NOT EXISTS idx_snapshot_version ON plan_snapshots(version_id)")
