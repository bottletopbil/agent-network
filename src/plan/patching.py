...
class PatchValidator:
    ...
    def validate_patch(self, patch: PlanPatch, current_plan: Optional[PlanStore] = None) -> Tuple[bool, Optional[str]]:
        """
        ...
        for idx, op_data in enumerate(patch.ops):
            ...
            if op_type_str == 'STATE':
                ...
                if 'parent' not in payload or 'child' not in payload:
                    return False, f"Op {idx}: LINK op missing 'parent' or 'child' in payload"
            ...
            for other_op in other.ops:
                ...
                if key in patch_ops:
                    ...
                    if other_op['op_type'] == 'STATE':
                        if our_op.get('payload', {}).get('state') != other_op.get('payload', {}).get('state'):
                            conflicts.append(
                ...
                if op_type == 'ADD_TASK':
                    ...
                    if task_id in add_task_ops:
                        ...
                        if patch.base_lamport > add_task_ops[task_id]['_patch_lamport']:
                            add_task_ops[task_id] = {**op_data, '_patch_lamport': patch.base_lamport, '_actor': patch.actor_id}
                    else:
                        add_task_ops[task_id] = {**op_data, '_patch_lamport': patch.base_lamport, '_actor': patch.actor_id}
                    ...
                    if key in merged_ops:
                        if patch.base_lamport > merged_ops[key]['_patch_lamport']:
                            merged_ops[key] = {**op_data, '_patch_lamport': patch.base_lamport, '_actor': patch.actor_id}
                    else:
                        merged_ops[key] = {**op_data, '_patch_lamport': patch.base_lamport, '_actor': patch.actor_id}
