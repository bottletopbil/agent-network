...
class PeerDiscovery:
    ...
    def __init__(
        ...
        logger.info(f"PeerDiscovery initialized: {local_peer_id} at {local_address}")
    ...
    async def announce_self(self) -> None:
        ...
        if self.nats_client:
            ...
            logger.debug(f"No NATS client - simulating announcement: {announcement}")
    ...
    async def discover_peers(self) -> List[PeerInfo]:
        ...
        if self.nats_client:
            ...
            try:
                # Query for peers
                await self.nats_client.publish(
                    "peers.query", json.dumps({"from": self.local_peer_id}).encode()
                )
