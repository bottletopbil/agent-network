...
class ChannelManager:
    ...
    def open_channel(self, sender: str, receiver: str, deposit: float) -> str:
        ...
        channel = PaymentChannel(
            channel_id=channel_id, sender=sender, receiver=receiver, capacity=deposit
        )
    ...
    def send_payment(self, channel_id: str, amount: float, nonce: int, signature: str) -> bool:
        ...
        if nonce <= channel.nonce:
            raise ValueError(f"Invalid nonce: {nonce} (current: {channel.nonce})")
    ...
    def get_channels_by_sender(self, sender: str) -> list[PaymentChannel]:
        ...
        return [ch for ch in self.channels.values() if ch.sender == sender]

    def get_channels_by_receiver(self, receiver: str) -> list[PaymentChannel]:
        """Get all channels where receiver is the receiver."""
        return [ch for ch in self.channels.values() if ch.receiver == receiver]
