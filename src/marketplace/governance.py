...
class GovernanceSystem:
    ...
    def submit_proposal(
        ...
        if stake < self.min_stake:
            raise ValueError(
                f"Insufficient stake: {stake} < {self.min_stake}"
            )
    ...
    def tally_votes(self, proposal_id: str) -> Dict[str, Any]:
        ...
        quorum_met = (total_votes / self.total_voting_weight) >= self.quorum_percentage if self.total_voting_weight > 0 else False
        ...
        approval_met = (yes_votes / votes_for_approval) >= self.approval_threshold if votes_for_approval > 0 else False
