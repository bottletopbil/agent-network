...
class WinnerSelector:
    ...
    def select_winner(self, canary_results: List[CanaryResult]) -> Optional[str]:
        """
    ...
    def select_winner_with_score(
        ...
        winner_result = next(r for r in canary_results if r.agent_id == winner_id)
    ...
    def rank_all(self, canary_results: List[CanaryResult]) -> List[Tuple[str, float]]:
        """
    ...
    def get_stats(self, canary_results: List[CanaryResult]) -> dict:
        ...
        """
        if not canary_results:
            return {"total": 0, "passed": 0, "qualified": 0, "pass_rate": 0.0}
