...
class CanaryRunner:
            ...
            try:
                if self.receive_result_handler:
                    output = await asyncio.wait_for(
                        self.receive_result_handler(agent_id),
                ...
                # Score quality
                quality_score = self._score_output(
                    output,
                    canary_test.expected_output
                )
    ...
    def _score_correctness(self, output: Any, expected: Any) -> float:
        ...
        if isinstance(output, dict) and isinstance(expected, dict):
            ...
            value_matches = sum(
                ...
                if output.get(k) == expected.get(k)
            )
    ...
    async def run_canaries(
        ...
        """
        tasks = [
            self.run_canary(agent_id, canary_test)
