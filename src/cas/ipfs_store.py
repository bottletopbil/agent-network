...
class IPFSContentStore:
    ...
    def get(self, cid: str, timeout: int = 5) -> bytes:
        ...
        if thread.is_alive():
            ...
            logger.warning(f"IPFS get timeout after {timeout}s (failure {self._failure_count}/{self._max_failures})")
        ...
        if error[0]:
            self._failure_count += 1
            logger.warning(f"IPFS get error (failure {self._failure_count}/{self._max_failures}): {error[0]}")
