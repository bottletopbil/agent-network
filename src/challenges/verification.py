...
class ChallengeVerifier:
    ...
    def verify_schema_violation(
        ...
        for field in claimed_violations:
            ...
            if actual_value is None:
                ...
                violations_found.append(f"{field}: expected {expected_type}, got {type(actual_value).__name__}")
        ...
        return VerificationResult(
            ...
            reason=f"Found {len(violations_found)} schema violations" if is_valid else "No violations found",
    ...
    def verify_missing_citation(
        ...
        return VerificationResult(
            is_valid=is_valid,
            gas_used=min(gas_used, self.gas_limit),
            reason=f"Found {len(missing_citations)} missing citations" if is_valid else "All citations present",
    ...
    def verify_semantic_contradiction(
        ...
        return VerificationResult(
            is_valid=is_valid,
            gas_used=min(gas_used, self.gas_limit),
            reason=f"Found {len(contradictions_found)} contradictions" if is_valid else "No contradictions found",
    ...
    def verify_output_mismatch(
        ...
        return VerificationResult(
            is_valid=is_valid,
            gas_used=min(gas_used, self.gas_limit),
            reason=f"Found {len(mismatches_found)} output mismatches" if is_valid else "Output matches specification",
    ...
    def verify_policy_breach(
        ...
        if not policy_rule:
            return VerificationResult(
                is_valid=False,
                gas_used=gas_used,
                reason="No policy rule specified"
            )
        ...
        return VerificationResult(
            is_valid=is_valid,
            gas_used=min(gas_used, self.gas_limit),
            reason=f"Policy breach confirmed: {policy_rule}" if is_valid else "No policy breach detected",
