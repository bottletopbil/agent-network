...
class AbuseDetector:
    ...
    def check_rate_limit(self, challenger_id: str) -> tuple[bool, Optional[str]]:
        ...
        if challenges_last_hour >= self.MAX_CHALLENGES_PER_HOUR:
            return False, f"Rate limit exceeded: {challenges_last_hour} challenges in last hour (max: {self.MAX_CHALLENGES_PER_HOUR})"
        ...
        if challenges_last_day >= self.MAX_CHALLENGES_PER_DAY:
            return False, f"Rate limit exceeded: {challenges_last_day} challenges in last 24h (max: {self.MAX_CHALLENGES_PER_DAY})"
    ...
    def check_spam_pattern(self, challenger_id: str) -> tuple[bool, Optional[str]]:
        ...
        if recent_challenges >= self.SPAM_THRESHOLD_COUNT:
            return True, f"Spam detected: {recent_challenges} challenges in {self.SPAM_DETECTION_WINDOW_SECONDS}s"
