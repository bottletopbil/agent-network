...
class EscalationHandler:
    ...
    def check_escalation_needed(
        ...
        if disagreement_rate >= self.DISAGREEMENT_THRESHOLD:
            return True, EscalationLevel.VERIFIER_CONSENSUS, f"Verifier disagreement: {disagreement_rate:.1%}"
        
        # Check confidence
        avg_confidence = sum(v.confidence for v in verdicts) / total_count
        if avg_confidence < self.CONFIDENCE_THRESHOLD:
            return True, EscalationLevel.HUMAN_REVIEW, f"Low confidence: {avg_confidence:.1%}"
        
        # Check high-value bonds
        if bond_amount >= self.HIGH_VALUE_BOND_THRESHOLD:
            return True, EscalationLevel.GOVERNANCE_VOTE, f"High-value bond: {bond_amount} credits"
    ...
    def create_human_review_task(self, challenge_id: str) -> str:
        ...
        print(f"[ESCALATION] Created human review task {review_task_id} for challenge {challenge_id}")
    ...
    def get_pending_escalations(self, level: Optional[EscalationLevel] = None) -> List[EscalationCase]:
