...
class ChallengeQueue:
    ...
    def _init_schema(self):
        ...
        with self.lock:
            with self.conn:
                ...
                """)
                self.conn.execute("CREATE INDEX IF NOT EXISTS idx_queue_priority ON challenge_queue(priority_score DESC, queued_at_ns ASC)")
                self.conn.execute("CREATE INDEX IF NOT EXISTS idx_queue_status ON challenge_queue(status)")
        ...
        with self.lock:
            with self.conn:
                self.conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'queued')
                """, (challenge_id, task_id, commit_id, challenger_id, proof_json,
                      bond_amount, queued_at_ns, priority_score))
        
        print(f"[QUEUE] Added challenge {challenge_id}, bond={bond_amount}, priority={priority_score:.2f}")
    ...
    def prioritize_by_bond(self) -> List[QueuedChallenge]:
        ...
        with self.lock:
            ...
            challenges = []
            for row in cursor:
                challenges.append(QueuedChallenge(
                    challenge_id=row[0],
                    task_id=row[1],
                    commit_id=row[2],
                    challenger_id=row[3],
    ...
    def get_queue_size(self, status: Optional[str] = None) -> int:
        ...
        with self.lock:
            if status:
                cursor = self.conn.execute(
                    "SELECT COUNT(*) FROM challenge_queue WHERE status = ?",
                    (status,)
                )
            else:
