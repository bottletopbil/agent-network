...
class ProofSchema:
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if self.size_bytes > MAX_PROOF_SIZE_BYTES:
            return False, f"Proof size {self.size_bytes} exceeds max {MAX_PROOF_SIZE_BYTES} bytes"
        ...
        if self.gas_estimate > MAX_GAS_ESTIMATE:
            return False, f"Gas estimate {self.gas_estimate} exceeds max {MAX_GAS_ESTIMATE}"
