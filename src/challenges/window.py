...
class ChallengeWindow:
    ...
    def _init_schema(self):
        ...
        with self.lock:
            with self.conn:
                ...
                """)
                self.conn.execute("CREATE INDEX IF NOT EXISTS idx_window_task ON challenge_windows(task_id)")
    ...
    def extend_window(self, task_id: str, extension_seconds: int) -> Optional[WindowInfo]:
        ...
        with self.lock:
            ...
            print(f"[CHALLENGE_WINDOW] Extended window for task {task_id} by {extension_seconds}s "
