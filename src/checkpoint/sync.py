...
class FastSync:
    ...
    def download_checkpoint(
        self, checkpoint_id: int, source_url: Optional[str] = None
    ) -> Optional[bytes]:
        """
        ...
        logger.info(f"Downloaded checkpoint {checkpoint_id}, " f"{len(data)} bytes")
    ...
    def sync_ops_after_epoch(
        ...
        logger.info(f"Synced {len(ops)} operations after epoch {epoch}")

        return ops

    def verify_continuity(self, checkpoint: SignedCheckpoint, ops: List[Dict]) -> bool:
        """
    ...
    def fast_sync_node(
        self, target_epoch: Optional[int] = None, op_source: Optional[callable] = None
    ) -> Optional[Dict]:
        """
        ...
        checkpoint_data = self.download_checkpoint(checkpoint.checkpoint.epoch)
        ...
        ops = self.sync_ops_after_epoch(checkpoint.checkpoint.epoch, op_source)
