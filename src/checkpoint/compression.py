...
class DeterministicCompressor:
    ...
    def __init__(self, compression_level: int = 3):
        ...
        self.compressor = zstd.ZstdCompressor(
            level=compression_level, write_content_size=True, write_checksum=True
        )
    ...
    def compress_state(self, plan_state: Dict) -> bytes:
        ...
        canonical_json = json.dumps(plan_state, sort_keys=True, separators=(",", ":"))
    ...
    def compress_thread(self, thread_ops: List[Dict]) -> Dict:
        ...
        canonical_json = json.dumps(thread_ops, sort_keys=True, separators=(",", ":"))
        data_bytes = canonical_json.encode("utf-8")
    ...
    def verify_compressed(self, original: Any, decompressed: Any) -> bool:
        """
    ...
    def compress_batch(
        ...
        for i in range(0, len(items), max_batch_size):
            ...
            for item in batch:
                canonical_json = json.dumps(item, sort_keys=True, separators=(",", ":"))
                compressed = self.compressor.compress(canonical_json.encode("utf-8"))
                compressed_items.append(compressed)

        logger.info(f"Compressed batch: {len(items)} items")
