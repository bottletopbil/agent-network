...
async def handle_attest_plan(envelope: dict):
    ...
    if verdict != "approve":
        print(
            f"[ATTEST_PLAN] Rejection from {sender[:8]}... for proposal {proposal_id} (not counted)"
        )
        return
    ...
    quorum_reached = quorum_tracker.record_attestation(
        need_id=need_id, proposal_id=proposal_id, verifier_id=sender, k_plan=k_plan
    )
    ...
    print(
        f"[ATTEST_PLAN] Recorded approval from {sender[:8]}... for proposal {proposal_id} ({attestation_count}/{k_plan})"
    )

    # If quorum reached, trigger DECIDE
    if quorum_reached:
        print(
            f"[ATTEST_PLAN] ✓ Quorum reached for proposal {proposal_id} ({k_plan}/{k_plan} approvals)"
        )
        ...
        if decide_record is None:
            ...
            print(
                f"[ATTEST_PLAN] CONFLICT ({consensus_type}): DECIDE already exists for need {need_id}"
            )
            return

        consensus_type = "Raft" if use_raft_consensus() else "Redis"
        print(
            f"[ATTEST_PLAN] ✓ DECIDE recorded ({consensus_type}): need {need_id} → proposal {proposal_id}"
        )
