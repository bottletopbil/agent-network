...
async def handle_attest(envelope: dict):
    ...
    eval_record = create_eval_record(
        policy_input, policy_decision, decision.policy_hash or "unknown"
    )
    ...
    logger.info(
        f"[ATTEST] Recorded attestation {attestation_id} for commit {commit_id} (verdict: {verdict})"
    )
    ...
    print(
        f"[ATTEST] {len(attestations)} attestation(s) for commit {commit_id} (threshold: {K_PLAN})"
    )
