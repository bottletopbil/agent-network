...
async def handle_heartbeat(envelope: dict):
    ...
    if lease_manager is None:
        ...
        if lease.worker_id != worker_id:
            print(
                f"[HEARTBEAT] ERROR: Worker mismatch for lease {lease_id}: expected {lease.worker_id}, got {worker_id}"
            )
            return
    ...
    print(
        f"[HEARTBEAT] Received from {worker_id[:8]}... for lease {lease_id} (progress: {progress}%)"
    )
