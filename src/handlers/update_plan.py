    ...
    for idx, op_data in enumerate(ops_data):
        ...
        if op_type_str == 'LINK':
            if 'parent' not in op_payload or 'child' not in op_payload:
                print(f"[UPDATE_PLAN] WARNING: Skipping op {idx}: LINK op missing 'parent' or 'child'")
                continue
    ...
    if applied_count > 0:
        try:
            ...
            if plan_state:
                ...
                print(f"[UPDATE_PLAN] Recorded version {version.version_id} at lamport {version.lamport}, merkle: {version.merkle_root[:16]}...")
