...
from challenges.proofs import ProofType, ProofSchema, MAX_PROOF_SIZE_BYTES, MAX_GAS_ESTIMATE
...
async def handle_challenge(envelope: dict):
    ...
    print(f"[CHALLENGE] Required bond: {required_bond} credits ({proof_type.value}, {complexity.name})")
    ...
    if ledger:
        ...
        if challenger_balance < required_bond:
            print(f"[CHALLENGE] ERROR: Insufficient balance: {challenger_balance} < {required_bond}")
            return
    ...
    print(f"[CHALLENGE] Proof validated: {proof_type.value}, size={size_bytes}B, gas={gas_estimate}")
    ...
    print(f"[CHALLENGE] Challenger: {challenger_id}, Bond: {required_bond}, Type: {proof_type.value}")
