...
class MessageCache:
    ...
    def cleanup(self):
        ...
        # Remove expired entries
        expired = [
            msg_id for msg_id, timestamp in self.cache.items()
...
class HybridBus:
    ...
    def __init__(
        ...
        logger.info(
            f"Hybrid bus initialized (P2P primary: {self.p2p_primary})"
        )
    ...
    def subscribe_envelopes(
        ...
        self.p2p_bus.subscribe_envelopes(
            subject,
            lambda env: deduplicated_handler(env, "P2P")
        )
        ...
        if self.nats_bus:
            self.nats_bus.subscribe_envelopes(
                subject,
                lambda env: deduplicated_handler(env, "NATS")
            )
