...
class PayoutDistributor:
    ...
    def calculate_shares(
        ...
        if not challenger:
            ...
            for i, member_id in enumerate(committee):
                ...
                shares.append(PayoutShare(
                    recipient_id=member_id,
                    amount=amount,
                    share_type="COMMITTEE"
                ))
        else:
    ...
    def can_payout(
        ...
        if elapsed < required_wait:
            return False, f"Challenge period not elapsed: {elapsed} < {required_wait} ns"
    ...
    def execute_payout(
        ...
        for share in shares:
            if share.share_type != "BURN":
                ...
                recipients[share.recipient_id] = recipients.get(share.recipient_id, 0) + share.amount
