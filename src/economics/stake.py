...
class StakeManager:
    ...
    def _init_schema(self):
        ...
        with self.conn:
            ...
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_unbonding_account ON unbonding(account_id)"
            )
            self.conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_unbonding_completed ON unbonding(completed)"
            )
    ...
    def get_unbonding_records(
        self, account_id: str, include_completed: bool = False
    ) -> List[UnbondingRecord]:
        """
