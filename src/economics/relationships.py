...
class RelationshipDetector:
    ...
    def detect_any_relationship(
        ...
        """
        return (
            self.detect_same_org(verifiers, challenger) or
            self.detect_same_asn(verifiers, challenger) or
            self.detect_identity_links(verifiers, challenger)
        )
    ...
    def get_related_parties(
        ...
        for verifier_id in verifiers:
            ...
            # Check organization
            if (challenger_info.org_domain and verifier_info.org_domain and
                challenger_info.org_domain == verifier_info.org_domain):
                related.add(verifier_id)
                continue
            
            # Check ASN
            if (challenger_info.asn is not None and verifier_info.asn is not None and
                challenger_info.asn == verifier_info.asn):
                related.add(verifier_id)
                continue
            
            # Check identity
            if (challenger_info.identity_hash and verifier_info.identity_hash and
                challenger_info.identity_hash == verifier_info.identity_hash):
                related.add(verifier_id)
