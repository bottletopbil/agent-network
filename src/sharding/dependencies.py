...
class DependencyDAG:
    ...
    def add_dependency(
        ...
        logger.debug(
            f"Added dependency: shard {from_shard} -> {to_shard} "
            f"for need {need_id}"
        )
    ...
    def mark_shard_complete(self, shard_id: int) -> Set[int]:
        ...
        logger.debug(
            f"Shard {shard_id} completed, {len(newly_ready)} shards now ready"
        )
    ...
    def get_ready_shards(self) -> List[int]:
        ...
        ready = [
            shard for shard, degree in self.in_degree.items()
    ...
    def topo_sort_shards(self) -> Optional[List[int]]:
        ...
        queue = deque([
            shard for shard in self.graph.keys()
            if in_degree_copy.get(shard, 0) == 0
        ])
...
class RollbackHandler:
    ...
    def salvage_partial_work(
        ...
        logger.info(
            f"Salvaged {len(salvaged)} artifacts from shard {shard_id}"
        )
