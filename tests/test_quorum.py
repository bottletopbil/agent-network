...
def test_quorum_tracking():
    ...
    result = tracker.record_attestation(
        need_id="need-1", proposal_id="prop-A", verifier_id="v1", k_plan=3
    )
    ...
    # Second attestation
    result = tracker.record_attestation(
        need_id="need-1", proposal_id="prop-A", verifier_id="v2", k_plan=3
    )
    ...
    result = tracker.record_attestation(
        need_id="need-1", proposal_id="prop-A", verifier_id="v3", k_plan=3
    )
...
def test_quorum_state_class():
    ...
    state = QuorumState(need_id="need-1", proposal_id="prop-A", k_plan_required=2)
