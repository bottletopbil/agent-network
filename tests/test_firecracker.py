...
def executor():
    ...
    return SandboxedExecutor(default_image="/mock/rootfs.img", mock_mode=True)
...
class TestResourceLimits:
    ...
    def test_valid_limits(self):
        ...
        limits = ResourceLimits(cpu_cores=2, mem_mb=512, disk_mb=2048, net_bw_mbps=100)
...
class TestFirecrackerVM:
    ...
    def test_start_vm_with_custom_resources(self, firecracker):
        ...
        limits = ResourceLimits(cpu_cores=4, mem_mb=1024, disk_mb=4096, net_bw_mbps=500)
...
class TestSandboxedExecutor:
    ...
    def test_execute_sandboxed_with_resources(self, executor):
        ...
        limits = ResourceLimits(cpu_cores=2, mem_mb=512)

        result = executor.execute_sandboxed("echo Test", resources=limits)

        assert result["exit_code"] == 0

    def test_execute_sandboxed_timeout(self, executor):
        """Test execution with timeout."""
        result = executor.execute_sandboxed("echo Quick", timeout=5)
...
class TestEdgeCases:
    ...
    def test_minimal_resources(self):
        ...
        limits = ResourceLimits(cpu_cores=1, mem_mb=64, disk_mb=100, net_bw_mbps=1)
