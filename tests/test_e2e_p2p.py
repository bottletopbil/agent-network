...
async def test_10_node_mesh(test_nodes_10):
    ...
    for sender_id, sender_node in enumerate(nodes):
        ...
        receivers = [i for i, node in enumerate(nodes) if i != sender_id and len(node.received_messages) > 0]
        propagation_rate = len(receivers) / (len(nodes) - 1) * 100
        
        logger.info(f"Node {sender_id} → {len(receivers)}/{len(nodes)-1} nodes ({propagation_rate:.1f}%)")
...
async def test_concurrent_publishers():
    ...
    for node in nodes:
        ...
        assert received_count >= total_expected * 0.8, \
