...
def test_late_bid_extends_auction_window():
    ...
    result = manager.accept_bid(task_id=task_id, agent_id="sniper", bid_amount=150)
    ...
    assert (
        new_deadline - original_deadline >= 4
    ), "Should extend by approximately 5 seconds"
...
def test_early_bid_does_not_extend():
    ...
    assert (
        abs(new_deadline - original_deadline) < 2
    ), "Early bid should not extend deadline"
