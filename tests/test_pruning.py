...
from checkpoint.pruning import PruningPolicy, TieredStorage, PruningManager
...
class TestIntegration:
    ...
    def test_full_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            manager = PruningManager(
                policy=PruningPolicy(keep_epochs=5), storage=TieredStorage(Path(tmpdir))
            )
    ...
    def test_stats_after_pruning(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            manager = PruningManager(
                policy=PruningPolicy(keep_epochs=3), storage=TieredStorage(Path(tmpdir))
            )
